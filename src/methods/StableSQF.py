...
class LitNBEATSQF(LightningModule):
    ...
    def __init__(self,
                 ...
                 lambda_stability: Union[float, Literal['nashmtl']] = 0.0,
                 enforce_nonnegative_forecasts_metric_calculation: bool = False,
                 optim_niter: int = 20, # nashmtl hyperparameter
                 update_weights_every: int = 1, # nashmtl hyperparameter
        ...
        if (not isinstance(lambda_stability, float)) and (lambda_stability != 'nashmtl'):
            raise ValueError("Invalid argument: lambda_stability must be a float or the string 'nashmtl'")
        ...
        # Init weight_methods for DLW (also to allow for model finetuning with DLW when not used initially)
        self.weight_method = WeightMethods(method='nashmtl', n_tasks=2, device=self.device,
    ...
    def _get_losses(self,
        ...
        if not calculate_metrics:
            ...
            if lambda_stability == 'nashmtl': # NASHMTL
                if not evaluate:
                    if (self.global_step != 0) and (self.global_step % 2 == 0):
                        loss, extra_outputs = self.weight_method.get_weighted_loss(
                            losses=losses,
                            shared_parameters=list(self.parameters()),
                        )
                        w_normalizing_constant = (extra_outputs["weights"][0]+extra_outputs["weights"][1])
                        w_quality = extra_outputs["weights"][0]/w_normalizing_constant
                        w_stability = extra_outputs["weights"][1]/w_normalizing_constant
                    else:
                        loss = losses[0]
                        w_quality = 1
                        w_stability = 0
                    ...
                    # model pmmgf50q
                    # quality
                    # 0.857257068157196 # vloss_q
                    # stability
                    # 0.6825957894325256 # vloss_s1
                    # 0.10840365290641785 # vloss_wcs1
                    # 0.2489810585975647 # vloss_wts1
                    if wc_stability:
                        loss = (-1)*(((0.857257068157196-losses[0])/0.857257068157196) + 
                                ((0.10840365290641785-losses[1])/0.10840365290641785))
                    elif wt_stability:
                        loss = (-1)*(((0.857257068157196-losses[0])/0.857257068157196) + 
                                ((0.2489810585975647-losses[1])/0.2489810585975647))
                    else:
                        loss = (-1)*(((0.857257068157196-losses[0])/0.857257068157196) + 
                                ((0.6825957894325256-losses[1])/0.6825957894325256))
            else: # TARW
                w_stability = self.hparams.lambda_stability
                #w_stability = torch.rand(1, device=self.device) * self.hparams.lambda_stability # Scales random value to [0, lambda_stability]
                w_quality = 1 - w_stability
                #loss = w_quality * (losses[1].detach()/(losses[0].detach()+losses[1].detach())) * losses[0] + w_stability * (losses[0].detach()/(losses[0].detach()+losses[1].detach())) * losses[1]
                loss = w_quality * losses[0] + w_stability * losses[1]
                # loss = losses[0] + self.hparams.lambda_stability * losses[1]
                # w_quality = 1
