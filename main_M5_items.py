...
if load_model:
    ...
    backcast_length_multiplier = 26 #27
    forecast_length = 14
    hidden_layer_units = 512 #1024 #256 #512 #128 #64
...
random_seed = 2956 #2512 #20 #2512 #20 #18
## Data hparams
forecasting_origin_range_multiplier = 1e6 #10
batch_size = 512 #512 #2048 #1024 #256 #128
...
# model-specific training and evaluation hparams below are ignored if (load_model == True) AND (update_loaded_model_specific_training_and_eval_hparams == False)
    ...
    wc_stability = True # wWx center
    wt_stability = False # wWx tails
    lambda_stability = 0.05 #0.1 #0.25 #0.20
...
if dataset == "M5":
    from src.data.M5 import load_data
...
if eval_mode == 'validation':
    ...
    if max_epochs == 0:
        ...
        #trainer.test(NBEATSQF, validation_dataloader_target, "last", verbose=True) # metrics are saved in ./wandb/run-.../files/output.log
        trainer.test(NBEATSQF, validation_dataloader_target, "best", verbose=True) # metrics are saved in ./wandb/run-.../files/output.log
elif eval_mode ==  'test':
    # Init additional callbacks that will be used in Trainer
    ...
    if max_epochs == 0:
        ...
        trainer.test(NBEATSQF, test_dataloader_target, "last", verbose=True) # metrics are saved in ./wandb/run-.../files/output.log
